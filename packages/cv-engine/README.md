...
Device strings are validated before hardware discovery. Accepted explicit values are `cpu`, `cuda`, and `cuda:<non-negative index>` such as `cuda:0` or `cuda:1`; `auto` is also supported. Values such as `gpu`, `cuda:`, `cuda:-1`, `cuda:abc`, and `cuda:0:1` are rejected. Valid syntax does not prove that CUDA hardware exists: a valid CUDA request falls back to CPU when PyTorch/CUDA is unavailable, while an out-of-range index is rejected when CUDA is available.
