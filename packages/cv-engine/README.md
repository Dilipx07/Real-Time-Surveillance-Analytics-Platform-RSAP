...
`ResilientCapture` handles webcams, files, and RTSP sources with low-latency OpenCV settings and scheduled reconnect attempts. Calls made during reconnect backoff wait on an interruptible event without holding the capture lock, so a disconnected camera cannot create a tight CPU loop and shutdown remains prompt.
